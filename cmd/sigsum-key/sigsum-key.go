...
`
	...
	case "-v", "--version":
		...
		os.Exit(0)
	case "generate", "gen": // TODO: deprecate old "gen"
...
func (s *VerifySettings) parse(args []string) {
	const usage = `
Verify a signature with a given namespace.  The namespaces that are
used in the Sigsum system are 'sigsum.org/v1/tree-leaf' (signed
checksums) and 'sigsum.org/v1/submit-token' (rate limitting).  The
default behavior is to not use any namespace.
...
func (s *SignSettings) parse(args []string) {
	const usage = `
Create a signature with a given namespace. The namespaces that are
used in the Sigsum system are 'sigsum.org/v1/tree-leaf' (signed
checksums) and 'sigsum.org/v1/submit-token' (rate limitting).  The
default behavior is to not use any namespace.
