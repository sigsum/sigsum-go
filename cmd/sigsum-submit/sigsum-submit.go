...
func main() {
	...
	if policy != nil {
		config := submit.Config{
			...
			FilterCosignatures: settings.filterCosignatures,
...
`
	...
	set.FlagLong(&s.filterCosignatures, "filter-cosignatures", 0, "Remove cosignatures from witnesses not in the active policy")
