...
If a ".proof" file already exists, then it is verified without
performing any new add-leaf request.  Invalid proofs are rejected.
...
`
	...
	set.FlagLong(&s.leafHash, "leaf-hash", 0, "Output leaf hash and exit")
	set.FlagLong(&s.outputFile, "output", 'o', "Store output in a file, only works for a single input", "output-file")
	set.FlagLong(&s.outputDir, "output-dir", 'O', "Store output in a directory [current directory if any input files]", "output-dir")
	set.FlagLong(&s.diagnostics, "diagnostics", 'd', "Available levels: fatal, error, warning, info, debug", "log-level")
	set.FlagLong(&s.tokenDomain, "token-domain", 'D', "Domain name to use for rate-limiting; \"_sigsum_v1.\" will be prepended", "domain-name")
	set.FlagLong(&s.tokenKeyFile, "token-signing-key", 'R', "Private key in OpenSSH format to sign DNS rate-limit tokens; or a corresponding public key where the private part is accessed using the SSH agent protocol", "key-file")
